...
model.compile(optimizer=optimizer, loss=model.compute_loss, metrics=tf.metrics.SparseCategoricalAccuracy())  # can also use any keras loss fn
#%%
...
evalu = model.evaluate(val_dataset.batch(10))


# In[13]:


model.save("TFAlbertForSequenceClassification")


# In[14]:


labels_pred = model.predict(test_dataset.batch(10))
...
result_output.to_csv("TFAlbert.csv", index=False, quoting=3)
